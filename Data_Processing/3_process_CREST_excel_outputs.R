...
processed_load_path <- paste(root_path,'CREST/Processed_Load/',sep='')
...
return(climate)
 ...
 # Set devices by tier and define color palletes to use for plotting (Heating not used)
...
devices_by_tier['4']<-list(c("Iron", "Microwave", "Washing machine", "Spacer")) # Spacer included as otherwise pallete makes final device look white
devices_by_tier['5']<-list(c("Hob","Vacuum","Air Conditioning", "Spacer")) # Spacer included as otherwise pallete makes final device look white
...
myColors<-c(rev(brewer.pal(length(devices_by_tier[['1']]),colors_by_tier[['1']]))[1:length(devices_by_tier[['1']])],
		    ...
		    'grey')
	names(myColors) <-c(all_techs,'Other Appliances')
...
# Missing R function to sum every element of vector
	...
	load_data_by_tier<-lapply(tiers, function(tier){
		load_data_by_month<-lapply(months, function(month){
			load_data_by_daytype<-lapply(daytypes, function(daytype){
				...
				print(paste('Importing data for ',name_abbrev,sep=''))
				...
				df <- cbind(df,Cooling=d_df[['Electricity used by cooling system']],Heating=d_df[['Electricity used by heating system']],Lighting=d_df[['Lighting demand']],Internal_Temp=d_df[['Internal building node temperature']]) 
				...
				# Revise fridge load based on internal temperature (then drop internal temp from df)
				fridge_load_factor<-sapply(df$Internal_Temp,function(temp){
						if(temp < min(fridge_consumption_factor_temp_fitline[['x']]))
						{
							load_factor<-min(fridge_consumption_factor_temp_fitline[['y']])
						}
						# Judged data to be too sparse to justify end of the lowess fit -> keep 98th percentile value for higher meter densities
						else if(temp > max(fridge_consumption_factor_temp_fitline[['x']]))
						{
							load_factor<-max(fridge_consumption_factor_temp_fitline[['y']])
						}
						else {
							load_factor <- approx(fridge_consumption_factor_temp_fitline,xout=temp)[['y']]
						}
						return(load_factor)
					})

				df[['Fridge freezer']]<-fridge_load_factor*as.numeric(as.character(df[['Fridge freezer']]))

				# Fix to lighting, which appears to occaisionally be calibrated wrongly (something to do with bulbs sheet in CREST model)
				#df[which(df$Lighting > 8*3),][['Lighting']]
...
print('STEP 1b. GET PEAK LOAD BY VILLAGE/TIER/DWELLING')

peak_load_by_village_tier_dwelling<-lapply(villages, function(village){
	load_data_by_tier<-lapply(tiers, function(tier){
		print(paste('Getting peak load by customer for ',village,' tier ',tier,sep=''))
		peak_load_month_and_customer<-sapply(months, function(month){
			peak_load_daytype_and_customer<-sapply(daytypes, function(daytype){
				...
				peak_load_by_customer <- sapply(df_by_customer,function(df){
					return(max(rowSums(df[-1][-1])))
				})
				return(peak_load_by_customer)
			})
			return(apply(peak_load_daytype_and_customer, MARGIN =  1, FUN = max, na.rm = F))
		})
		return(apply(peak_load_month_and_customer, MARGIN =  1, FUN = max, na.rm = F))
	})	
	names(load_data_by_tier)<-tiers
	return(load_data_by_tier)
})
names(peak_load_by_village_tier_dwelling)<-villages

print('STEP 1c. GET AGGREGATED PEAK LOAD BY VILLAGE/TIER')

agg_peak_load_by_village_tier<-lapply(villages, function(village){
	load_data_by_tier<-lapply(tiers, function(tier){
		print(paste('Getting aggregated peak load for ',village,' tier ',tier,sep=''))
		peak_agg_load_month<-sapply(months, function(month){
			peak_agg_load_daytype<-sapply(daytypes, function(daytype){
				df<-load_data_by_village_tier_month_daytype[[village]][[as.character(tier)]][[as.character(month)]][[daytype]]
				customer_numbers <- unique(df[["Dwelling index"]])
				df_by_customer <- lapply(customer_numbers, split_load_CREST, data=df )

				# Convert data into hourly (mean load across each minute in each hour)
				load_by_customer <- lapply(df_by_customer,function(df){
					return(rowSums(df[-1][-1]))
				})

				agg_load<-do.call(psum, load_by_customer)
				return(max(agg_load))
			})
			return(max(peak_agg_load_daytype))
		})
		return(max(peak_agg_load_month))
	})	
	names(load_data_by_tier)<-tiers
	return(load_data_by_tier)
})
names(agg_peak_load_by_village_tier)<-villages


# print('STEP 1b. GET MEAN PEAK LOAD BY VILLAGE/TIER')

# mean_peak_by_village_tier<-lapply(villages, function(village){
# 	mean_peak_by_tier<-lapply(tiers, function(tier){
# 		print(paste('Getting average peak load for ',village,' tier ',tier,' (across households)',sep=''))
# 		mean_peak_by_month<-sapply(months, function(month){
# 			mean_peak_customer_load_by_daytype<-sapply(daytypes, function(daytype){
# 				df<-load_data_by_village_tier_month_daytype[[village]][[as.character(tier)]][[as.character(month)]][[daytype]]
# 				customer_numbers <- unique(df[["Dwelling index"]])
# 				df_by_customer <- lapply(customer_numbers, split_load_CREST, data=df )
...
# 					return(max(rowSums(df[-1][-1])))
# 				})
# 				return(mean(peak_load_by_customer))
# 			})
# 			return(max(mean_peak_customer_load_by_daytype))
# 		})
# 		return(max(mean_peak_by_month))
# 	})	
# 	names(mean_peak_by_tier)<-tiers
# 	return(mean_peak_by_tier)
# })
# names(mean_peak_by_village_tier)<-villages
...
# Generate data disaggregated across across N dwellings

hourly_load_data_by_village_tier_month_daytype_dwelling_index<-lapply(villages, function(village){
	load_data_by_tier<-lapply(tiers, function(tier){
	...
	load_data_by_tier<-lapply(tiers, function(tier){
		...
		load_data_by_dwelling<-lapply(dwelling_indices, function(dwelling){
			hour_load<-apply(time_data, 1, function(x){
				...
				return(hourly_load_data_by_village_tier_month_daytype_dwelling_index[[village]][[as.character(tier)]][[as.character(month)]][[daytype]][[dwelling]][-1][-1][hour+1,])
				#return(sum(hourly_load_data_by_village_tier_month_daytype_dwelling_index[[village]][[as.character(tier)]][[as.character(month)]][[daytype]][[dwelling]][-1][-1][hour+1,]))
			})
...
load_stats_by_village_tier<-lapply(villages, function(village){
	...
	load_stats_daily_load_by_tier<-lapply(tiers, function(tier){
		...
		# PEAK LOAD
		peak_load_by_user <- peak_load_by_village_tier_dwelling[[village]][[as.character(tier)]]

		#aggregate(. ~ dwelling_index, data_combined[c('dwelling_index','total_load')], max)

		#peak_load_by_village_tier_dwelling Mean
		peak_load<-mean(peak_load_by_user)

		#mean(peak_load_by_user$total_load)

		# Stdev section commented out for peak load & load factor - used old way of generating load factor which took into account only hourly (rather than minute by minute) variation. I didn't rewrite this as echnically a bit challenging. Would require adaptation of part 1b.
		...
		max_aggregate_load<-agg_peak_load_by_village_tier[[village]][[as.character(tier)]]

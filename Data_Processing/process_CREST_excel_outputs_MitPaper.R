...
example_village='Bhinjpur' # Should replace with name of arbitrary location from which non-climatic appliance load data should be taken
...
load_data_by_tier_month_daytype<-lapply(tiers, function(tier){
	load_data_by_month<-lapply(months, function(month){
		load_data_by_daytype<-lapply(daytypes, function(daytype){
			...
			print('non climate dependent appliance data from this location will be used for all locations')
...
names(load_data_by_tier_month_daytype)<-tiers
...
hourly_load_data_by_tier_month_daytype_dwelling_index<-lapply(tiers, function(tier){
	print(paste('Converting data to hourly for tier ',tier,sep=''))
	load_data_by_month<-lapply(months, function(month){
		load_data_by_daytype<-lapply(daytypes, function(daytype){
			df<-load_data_by_tier_month_daytype[[as.character(tier)]][[as.character(month)]][[daytype]]
...
names(hourly_load_data_by_tier_month_daytype_dwelling_index)<-tiers

# Aggregrate across appliances

aggregated_hourly_load_data_by_tier_month_daytype_dwelling_index<-lapply(tiers, function(tier){
	print(paste('Converting data to hourly for tier ',tier,sep=''))
	load_data_by_month<-lapply(months, function(month){
		load_data_by_daytype<-lapply(daytypes, function(daytype){
			agg_hourly_data<-lapply(hourly_load_data_by_tier_month_daytype_dwelling_index[[as.character(tier)]][[as.character(month)]][[daytype]],function(df){
...
names(aggregated_hourly_load_data_by_tier_month_daytype_dwelling_index)<-tiers
...
annual_hourly_load_by_village_tier_dwelling<-lapply(villages, function(village){
	load_data_by_tier<-lapply(tiers, function(tier){
		...
		load_data_by_dwelling<-lapply(dwelling_indices, function(dwelling){
			hour_load<-apply(time_data, 1, function(x){
				...
				return(cbind(hourly_climatic_load_data_by_village_tier_month_daytype_dwelling_index[[village]][[as.character(tier)]][[as.character(month)]][[daytype]][[dwelling]][-1][-1][hour+1,],
							 Appliance_Load=aggregated_hourly_load_data_by_tier_month_daytype_dwelling_index[[as.character(tier)]][[as.character(month)]][[daytype]][[dwelling]][-1][-1][hour+1,]))
		...
		return(climatic_load_data_by_dwelling)
	})	
	names(climatic_load_data_by_tier)<-tiers
	return(climatic_load_data_by_tier)
})
names(annual_hourly_climatic_load_by_village_tier_dwelling)<-villages

head(annual_hourly_climatic_load_by_village_tier_dwelling[[village]][[as.character(tier)]][[1]])
